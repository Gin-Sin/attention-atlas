...
`assets/chapters.js` 必须按下列顺序提供且只提供 12 个 id：

`mha, mqa, gqa, mla, mfa, tpa, dsa, csa, hca, linear, gated-delta, kda`
...
位置部分不是通用 RoPE 模板，而是逐章说明原论文或报告实际采用的机制：原始 MHA 的加性正弦编码、MQA/GQA 与位置机制的正交关系、MLA/DSA 的 decoupled 或 partial RoPE、MFA 对逐头 query 与共享 key 的标准 RoPE、TPA 只旋转 B_Q/B_K channel 因子的逐行 RoPE、CSA/HCA 的 partial + inverse RoPE，以及 Linear/Delta/KDA 中由前缀、ShortConv、衰减和有序状态转移承担的时序信息。具体模型配方与架构定义会明确分开。
...
- `mla → pytorch/mla.py`：11 块，低秩 Q/KV、decoupled RoPE、latent cache；同一组权重的两条等价执行图——重建式（Prefill 形态，显式展开每头 K/V）与双侧吸收式（Decode 形态，K 上投影折进 query、V 上投影折进输出，从不物化历史多头 K/V）。
- `mfa → pytorch/mfa.py`：10 块，共享 C 维 K/V、逐头 C×C Q_c/O_c 变换、标准 RoPE、与 head 数无关的共享缓存、可选 key-reuse 变体与增量解码等价 smoke test。
- `tpa → pytorch/tpa.py`：10 块，六路 contextual A/B 因子、B_Q/B_K 逐行 RoPE、因子化 KV cache、因子域 score/value 收缩与重建验证路径。
...
运行 Python 单元测试和十二套 CPU 参考实现的微型前向测试：
...
- `assets/chapters.js`：十二章教学内容、位置机制、推导、练习与来源
- `assets/diagrams.js`：十二套技术报告级 Attention Block SVG 与编号导读
