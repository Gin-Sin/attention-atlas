...
- `dsa → pytorch/dsa.py`：4 块，partial RoPE、Indexer 对齐损失、稀疏路由与 smoke test。
- `csa → pytorch/csa.py`：4 块，重叠压缩、top-k、局部窗口与 smoke test。
- `hca → pytorch/hca.py`：4 块，非重叠重压缩、dense 摘要读取、局部窗口与 smoke test。
